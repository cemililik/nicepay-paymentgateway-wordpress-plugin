...
run_suffix="$$-$(date +%s)"
network_name="nicepay-it-net-${run_suffix}"
database_container="nicepay-it-db-${run_suffix}"
wordpress_container="nicepay-it-wp-${run_suffix}"
temporary_root="$(mktemp -d /tmp/nicepay-schema-it.XXXXXX)"
...
if [[ ! "$run_suffix" =~ ^[0-9]+-[0-9]+$ ]] ||
   [[ ! "$temporary_root" =~ ^/tmp/nicepay-schema-it\.[A-Za-z0-9]+$ ]] || [[ ! -d "$temporary_root" ]]; then
    echo 'ERROR: could not establish safe disposable integration-test targets' >&2
    exit 1
fi
...
cleanup() {
    ...
    if [[ -n "$temporary_root" && "$temporary_root" =~ ^/tmp/nicepay-schema-it\.[A-Za-z0-9]+$ && -d "$temporary_root" ]]; then
...
docker run --detach --name "$database_container" --network "$network_name" \
    --env MARIADB_DATABASE=nicepay_integration \
...
docker run --detach --name "$wordpress_container" --network "$network_name" \
    ...
    --env WORDPRESS_DB_NAME=nicepay_integration \
...
wp_cli=(docker run --rm --user 0 --volumes-from "$wordpress_container" --network "$network_name"
    --env NICEPAY_INTEGRATION_TEST=1
    --env WORDPRESS_DB_HOST="${database_container}:3306"
    --env WORDPRESS_DB_USER=nicepay
    --env WORDPRESS_DB_PASSWORD=nicepay-test-password
    --env WORDPRESS_DB_NAME=nicepay_integration
...
"${wp_cli[@]}" wp core install --url=https://nicepay.test --title=NicePay \
    --admin_user=admin --admin_password=integration-password --admin_email=admin@example.com --skip-email --allow-root
"${wp_cli[@]}" wp plugin install /tmp/nicepay-payment-gateway.zip --activate --allow-root
"${wp_cli[@]}" wp eval-file /plugin-source/tests/integration/schema-migration.php --allow-root
