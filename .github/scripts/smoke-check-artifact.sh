...
if [[ -z "$artifact_path" || ! -f "$artifact_path" ]]; then
    echo 'Usage: smoke-check-artifact.sh <artifact.zip>' >&2
    exit 1
fi
...
for forbidden_pattern in \
    ...
    '(^|/)docs/analysis(/|$)' \
    '(^|/)(composer\.(json|lock)|package(-lock)?\.json|eslint\.config\.js|phpunit\.xml|\.env)(/|$)'; do
...
done < <(find "$temporary_root/$plugin_slug/assets/js" -type f -name '*.js' -print0)
